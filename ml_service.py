...
class NFSSecurityMLService:
    ...
    def load_models(self, model_dir='nfs_ml_models'):
        """加载所有模型"""
        try:
            ...
            self.is_anomaly_trained = models_data.get('is_anomaly_trained', False)
            self.is_threat_trained = models_data.get('is_threat_trained', False)
            self.is_behavior_trained = models_data.get('is_behavior_trained', False)
            
            self.logger.info("模型加载成功")
            
        except FileNotFoundError:
            self.logger.info("未找到保存的模型文件")
        except Exception as e:
